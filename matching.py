...
class FeatureMatcher:
    ...
    def extract_sift_features(self, images: List[np.ndarray]) -> List[Tuple[List[cv2.KeyPoint], np.ndarray]]:
        """
    ...
    def find_raw_matches(self,
                         image_descriptors: List[np.ndarray]) -> Dict[Tuple[int, int], List[cv2.DMatch]]:
        """
        ...
        for i in range(num_images):
            for j in range(i + 1, num_images):
                ...
                if desc1.size == 0 or desc2.size == 0:
                    print(f"  Skipping match between image #{i} and #{j}: one or both have no descriptors.")
                    continue
                ...
                print(f"  Image #{i} and Image #{j}: {len(good_matches)} raw matches (after ratio test)")
        return raw_matches_map

    def filter_matches_geometric_consistency(self,
                                             raw_matches_map: Dict[Tuple[int, int], List[cv2.DMatch]],
                                             all_image_keypoints: List[List[cv2.KeyPoint]]) -> Dict[Tuple[int, int], List[cv2.DMatch]]:
        """
        ...
        for (idx1, idx2), matches_list in raw_matches_map.items():
            ...
            if len(matches_list) < self.min_inlier_matches:
                print(f"  Skipping RANSAC for Image #{idx1} and #{idx2}: too few raw matches ({len(matches_list)})")
                continue

            # Extract keypoint coordinates for matched points
            src_pts = np.float32([all_image_keypoints[idx1][m.queryIdx].pt for m in matches_list]).reshape(-1, 1, 2)
            dst_pts = np.float32([all_image_keypoints[idx2][m.trainIdx].pt for m in matches_list]).reshape(-1, 1, 2)

            # Estimate Fundamental Matrix using RANSAC
            fundamental_matrix, mask = cv2.findFundamentalMat(
                src_pts, dst_pts, cv2.FM_RANSAC, self.ransac_reprojection_threshold, 0.99)

            # Filter out matches based on the RANSAC mask
            if mask is None or fundamental_matrix is None or (fundamental_matrix.shape == (3, 3) and np.linalg.det(fundamental_matrix) > 1e-7):
                print(f"  Image #{idx1} and #{idx2}: RANSAC failed to find a reliable Fundamental Matrix.")
                continue

            inlier_matches = [matches_list[i] for i in range(len(matches_list)) if mask[i] == 1]

            if len(inlier_matches) >= self.min_inlier_matches:
                filtered_matches_map[(idx1, idx2)] = inlier_matches
                print(f"  Image #{idx1} and #{idx2}: {len(inlier_matches)} inlier matches (after RANSAC)")
            else:
                print(f"  Image #{idx1} and #{idx2}: Not enough inlier matches ({len(inlier_matches)}) after RANSAC.")

        return filtered_matches_map

    def build_connectivity_graph(self,
                                 num_images: int,
                                 filtered_matches: Dict[Tuple[int, int], List[cv2.DMatch]]) -> Tuple[np.ndarray, List[Tuple[int, int]]]:
        """
    ...
    def saving_plots_kpm(self, num_images, dataset_name, all_keypoints, filtered_matches, images, images_color_for_plotting):
            ...
            for j in range(i + 1, num_images):
                if filtered_matches[i,j] and len(filtered_matches[i,j]) > 0:
                    ...
                    plot_and_save_matches(
                        img1_for_plot, all_keypoints[i],
                        img2_for_plot, all_keypoints[j],
                        filtered_matches[i,j], # Draw the good (filtered) matches
                        os.path.join(matches_out_dir,
    ...
    def process_images(self, images: List[np.ndarray], save_plot=False, dataset_name="", images_color_for_plotting: List[np.ndarray]=[]) -> Dict[str, any]:
        """
        ...
        filtered_matches = self.filter_matches_geometric_consistency(raw_matches, all_keypoints)

        # Step 4: Build connectivity graph
        num_images = len(images)
        adjacency_matrix, connected_pairs = self.build_connectivity_graph(num_images, filtered_matches)
        ...
        if save_plot:
            self.saving_plots_kpm(num_images, dataset_name, all_keypoints, filtered_matches, images, images_color_for_plotting)
